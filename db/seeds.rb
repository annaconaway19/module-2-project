...
drink_ids.each do |id|
  drink = JSON.parse(RestClient.get('https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i=' + id))['drinks'][0]

  ct = Cocktail.create(name: drink['strDrink'], description: drink['strInstructions'])


  ingredient_keys = drink.keys.select { |key| key.start_with?('strIngredient') }
